...
var schema = require('./public/js/backend/schema');
var mongoModules = require('./public/js/backend/mongo_modules');
...
{
    mongoModules.uploadProfilePic(req, function(response)
    {
        res.send(response);
    });
});
...
{
    mongoModules.storeProfilePic(req, res);
});
...
io.on('connection',function(socket)
{
    ...
    { 
        ...
        {
            clientIdArray.push({'userEmail': userData.userEmail, companyName: userData.companyName,'clientId': socket.id});
        }
        ...
        for (var i=0; i<clientIdArray.length; i++)
        {
            if (clientIdArray[i].companyName == userData.companyName)
            {
                counter = counter + 1;
            }
        }
    ...
    {
        // var onlineUsers = clientIdArray.filter(function(key)
        //     {
        //         return key.clientId == socket.id;
        //     });
...
//////////////////////////////////////////////////////
// ***** REMOVE LOCAL COPY OF OLD PROFILE PIC ***** //
//////////////////////////////////////////////////////
app.post('/removeOldProfilePic', function(req, res)
{   
    var path = 'public/images/profile_pictures/' + req.body.oldImage;
    fs.unlink(path,function(err)
    {
        if (!err)
        {
            res.send('old profile pic removed');
        }
        else
        {
            res.send(err);
